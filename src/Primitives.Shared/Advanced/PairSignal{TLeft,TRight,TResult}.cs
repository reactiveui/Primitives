...
/// <summary>Combines paired values from two sources by index.</summary>
...
public sealed class PairSignal<TLeft, TRight, TResult> : IObservable<TResult>
{
    ...
    public PairSignal(IObservable<TLeft> left, IObservable<TRight> right, Func<TLeft, TRight, TResult> selector)
    {
        Left = left;
        Right = right;
        Selector = selector;
    ...
    public IDisposable Subscribe(IObserver<TResult> observer)
    {
        ...
        return new PairWitness<TLeft, TRight, TResult>(observer, Selector).Run(Left, Right);
    }
}
