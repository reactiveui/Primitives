...
public partial class ObservableSubscriptionExtensionsTests
{
    ...
    {
        var result = Observable.Return(SchedulerSentinelValue).WaitForValue(ImmediateSequencer.Instance);
    ...
    {
        var error = Observable.Return(SchedulerSentinelValue).WaitForError(ImmediateSequencer.Instance);
