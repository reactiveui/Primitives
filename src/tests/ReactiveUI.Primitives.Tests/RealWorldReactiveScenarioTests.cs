...
{
    /// <summary>
    /// A reused value of two for element indices and expected counts.
    /// </summary>
    private const int Two = 2;

    /// <summary>
    /// The fallback value emitted when a source is empty.
    /// </summary>
    private const string FallbackValue = "fallback";

    /// <summary>
    /// The search query used by the view-model projection scenario.
    /// </summary>
    private const string SearchQuery = "rx primitives";

    /// <summary>
    /// Expected fallback values emitted when a source is empty.
    /// </summary>
    private static readonly string[] FallbackValues = [FallbackValue];

    /// <summary>
    /// Expected error values surfaced by a broken source.
    /// </summary>
    private static readonly string[] BrokenErrors = ["broken"];

    /// <summary>
    /// Expected single-element true result for terminal predicate captures.
    /// </summary>
    ...
    public void TelemetryBurstBuffersTerminalCountsAndSubscriberChurnAreDeterministic()
    {
        ...
        Assert.Equal(new[] { ExpectedCriticalCount }, Capture(count));
