...
public partial class InternalInfrastructureCoverageTests
{
    ...
    /// <summary>
    /// The integer constant ninety-nine.
    /// </summary>
    private const int NinetyNine = 99;

    /// <summary>
    /// The timeout in seconds used when waiting for asynchronous branches.
    /// </summary>
    private const int TimeoutSeconds = 2;

    /// <summary>
    /// The delay in milliseconds between condition polls.
    /// </summary>
    ...
    /// <summary>
    /// The expected side-effect log produced by the tapped source and the faulted tap.
    /// </summary>
    private static readonly string[] ExpectedTapSideEffects =
        ["next:1", "next:2", "next:3", "next:4", "completed", "error:do-error"];

    /// <summary>
    /// The expected keys retained when distinct-by length is applied.
    /// </summary>
    private static readonly string[] ExpectedDistinctKeys = ["aa", "ccc", "dd", "e"];

    /// <summary>
    /// The expected emptiness results for the empty and non-empty sources.
    /// </summary>
    private static readonly bool[] ExpectedIsEmptyValues = [true, false];

    /// <summary>
    /// The expected values produced by the looped string signal.
    /// </summary>
    private static readonly string[] ExpectedRepeatValues = ["r", "r", "r"];

    /// <summary>
    /// The expected error type names produced by the task factory continuations.
    /// </summary>
    private static readonly string[] ExpectedTaskErrorNames =
        [nameof(InvalidOperationException), nameof(TaskCanceledException)];

    /// <summary>
    /// The expected single zero-tick emission produced by one-shot timer factories.
    /// </summary>
    private static readonly long[] ExpectedSingleZeroTick = [ZeroLong];

    /// <summary>
    /// The expected zero-through-two tick emissions produced by periodic timer factories.
    /// </summary>
    private static readonly long[] ExpectedZeroToTwoTicks = [ZeroLong, OneLong, TwoLong];

    /// <summary>
    /// The expected message from the single delayed-error branch.
    /// </summary>
    private static readonly string[] ExpectedDelayErrors = ["delay-error"];

    /// <summary>
    /// The expected error type name from the expire-timeout branch.
    /// </summary>
    private static readonly string[] ExpectedTimeoutErrors = [nameof(TimeoutException)];

    /// <summary>
    /// The expected single true value emitted by the true signal.
    /// </summary>
    private static readonly bool[] ExpectedTrueValues = [true];

    /// <summary>
    /// The expected single false value emitted by the false signal.
    /// </summary>
    private static readonly bool[] ExpectedFalseValues = [false];

    /// <summary>
    /// The expected message from the keep-predicate fault branch.
    /// </summary>
    private static readonly string[] ExpectedKeepErrors = ["keep-predicate"];

    /// <summary>
    /// The expected message from the all-predicate fault branch.
    /// </summary>
    private static readonly string[] ExpectedAllErrors = ["all-predicate"];

    /// <summary>
    /// The expected messages from the recover handler-fault and unmatched branches.
    /// </summary>
    private static readonly string[] ExpectedCatchErrors = ["handler-threw", "not-matched"];

    /// <summary>
    /// The expected value emitted by the scheduled return signal.
    /// </summary>
    private static readonly string[] ExpectedScheduledReturn = ["scheduled"];

    /// <summary>
    /// The expected message from the map-selector fault branch.
    /// </summary>
    private static readonly string[] ExpectedMappedErrors = ["map-fault"];

    /// <summary>
    /// The expected long count produced by the range-backed distinct long-count alias.
    /// </summary>
    private static readonly long[] ExpectedRangeDistinctLongCount = [TwoLong];

    /// <summary>
    /// The expected one-through-four sequence emitted by the four-element source.
    /// </summary>
    private static readonly int[] ExpectedOneToFour = [One, Two, Three, Four];

    /// <summary>
    /// The expected single null produced by the default-if-empty branch.
    /// </summary>
    private static readonly int?[] ExpectedSingleNull = [null];

    /// <summary>
    /// The expected one-and-two prefix retained by take-while and distinct branches.
    /// </summary>
    private static readonly int[] ExpectedOneTwo = [One, Two];

    /// <summary>
    /// The expected three-and-four suffix retained by skip-while and delay branches.
    /// </summary>
    private static readonly int[] ExpectedThreeFour = [Three, Four];

    /// <summary>
    /// The expected single nine produced by the fork-join sum branch.
    /// </summary>
    private static readonly int[] ExpectedSingleNine = [Nine];

    /// <summary>
    /// The expected three-through-five sequence emitted by the range factory.
    /// </summary>
    private static readonly int[] ExpectedThreeToFive = [Three, Four, Five];

    /// <summary>
    /// The expected repeated five values emitted by the bounded loop factory.
    /// </summary>
    private static readonly int[] ExpectedFiveFive = [Five, Five];

    /// <summary>
    /// The expected single seven produced by the start and scheduled branches.
    /// </summary>
    private static readonly int[] ExpectedSingleSeven = [Seven];

    /// <summary>
    /// The expected single one observed by retained subscribers.
    /// </summary>
    private static readonly int[] ExpectedSingleOne = [One];

    /// <summary>
    /// The expected single five observed by retained subscribers.
    /// </summary>
    private static readonly int[] ExpectedSingleFive = [Five];

    /// <summary>
    /// The expected two-through-four prefix produced by the single prepend branch.
    /// </summary>
    private static readonly int[] ExpectedTwoToFour = [Two, Three, Four];

    /// <summary>
    /// The expected three repeated sevens produced by the bounded loop signal.
    /// </summary>
    private static readonly int[] ExpectedSevenSevenSeven = [Seven, Seven, Seven];

    /// <summary>
    /// The expected paired sums produced by the zip alias.
    /// </summary>
    private static readonly int[] ExpectedFiveSevenNine = [Five, Seven, Nine];

    /// <summary>
    /// The expected two-and-three sequence produced by the observer prepend branch.
    /// </summary>
    private static readonly int[] ExpectedTwoThree = [Two, Three];

    /// <summary>
    /// The expected one-through-three sequence produced by the prepend/append branches.
    /// </summary>
    private static readonly int[] ExpectedOneToThree = [One, Two, Three];

    /// <summary>
    /// The expected single two produced by the distinct count alias.
    /// </summary>
    private static readonly int[] ExpectedSingleTwo = [Two];

    /// <summary>
    /// The expected three-and-five sequence produced by the combine-latest branch.
    /// </summary>
    private static readonly int[] ExpectedThreeFive = [Three, Five];

    /// <summary>
    /// Asserts the argument guards exposed by the parity operator surface.
    /// </summary>
    ...
    {
        Assert.Throws<ArgumentNullException>(() => LinqMixins.Prepend(null!, One, Two));
        ...
        Assert.Throws<ArgumentNullException>(() => LinqMixins.Contains(null!, One));
    ...
    /// <summary>
    /// Covers <see cref="AsyncSignal{T}"/> subscriber churn, late subscriptions, disposal, and terminal no-op branches.
    /// </summary>
    ...
    /// <summary>
    /// Polls a condition until it succeeds or the timeout elapses.
    /// </summary>
    ...
    /// <summary>
    /// An equality comparer that throws when comparing values.
    /// </summary>
    private sealed class ThrowingComparer : IEqualityComparer<int>
    {
        /// <summary>
        /// Defers to a faulting comparison so the equality comparison throws when invoked.
        /// </summary>
        ...
        /// <summary>
        /// Returns the hash code for the specified value.
        /// </summary>
        ...
        /// <summary>
        /// Throws to simulate a faulting comparison.
        /// </summary>
    ...
    /// <summary>
    /// An observable that replays a scripted sequence of observer callbacks on subscribe.
    /// </summary>
    ...
    {
        /// <summary>
        /// The scripted callback invoked against each subscribing observer.
        /// </summary>
        ...
        /// <summary>
        /// Subscribes the observer and replays the scripted callback.
        /// </summary>
    ...
    /// <summary>
    /// A minimal virtual-time sequencer used to exercise scheduling edge branches.
    /// </summary>
    private sealed class MinimalVirtualClock : VirtualTimeSequencerBase<long, long>
    {
        /// <summary>
        /// The scheduled work items keyed by their absolute due time.
        /// </summary>
        private readonly SortedDictionary<long, Queue<Scheduled>> _scheduled = [];

        /// <summary>
        /// Initializes a new instance of the <see cref="MinimalVirtualClock"/> class.
        /// </summary>
        ...
        /// <summary>
        /// Schedules an action at the specified absolute due time.
        /// </summary>
        ...
        /// <summary>
        /// Adds a relative offset to an absolute time.
        /// </summary>
        ...
        /// <summary>
        /// Returns the next non-cancelled scheduled item, if any.
        /// </summary>
        ...
        /// <summary>
        /// Converts an absolute time to a <see cref="DateTimeOffset"/>.
        /// </summary>
        ...
        /// <summary>
        /// Converts a time span to the relative tick representation.
        /// </summary>
        ...
        /// <summary>
        /// A single scheduled work item tracked by the virtual clock.
        /// </summary>
        private sealed class Scheduled : IScheduledItem<long>
        {
            /// <summary>
            /// The action to run when the item is invoked.
            /// </summary>
            private readonly Func<IDisposable> _action;

            /// <summary>
            /// Initializes a new instance of the <see cref="Scheduled"/> class.
            /// </summary>
            ...
            /// <summary>
            /// Gets the absolute due time for the item.
            /// </summary>
            ...
            /// <summary>
            /// Invokes the scheduled action unless the item has been cancelled.
            /// </summary>
            public void Invoke()
            {
                if (IsCancelled)
    ...
    /// <summary>
    /// An observer that can be configured to throw on specific callbacks.
    /// </summary>
    /// <typeparam name="T">The type of the observed values.</typeparam>
    private sealed class ThrowingObserver<T> : IObserver<T>
    {
        /// <summary>
        /// A value indicating whether to throw on <see cref="OnNext"/>.
        /// </summary>
        private readonly bool _throwOnNext;

        /// <summary>
        /// A value indicating whether to throw on <see cref="OnError"/>.
        /// </summary>
        private readonly bool _throwOnError;

        /// <summary>
        /// A value indicating whether to throw on <see cref="OnCompleted"/>.
        /// </summary>
        ...
        /// </summary>
        /// <param name="throwOnNext">Whether to throw on <see cref="OnNext"/>.</param>
        /// <param name="throwOnError">Whether to throw on <see cref="OnError"/>.</param>
        /// <param name="throwOnCompleted">Whether to throw on <see cref="OnCompleted"/>.</param>
        ...
        /// <summary>
        /// Gets a value indicating whether an error callback has been observed.
        /// </summary>
    ...
    /// <summary>
    /// A scheduled item probe that delegates invocation to a supplied factory.
    /// </summary>
    private sealed class ScheduledProbe : ScheduledItem<int>
    {
        /// <summary>
        /// The factory invoked when the item runs.
        /// </summary>
        private readonly Func<IDisposable> _invoke;

        /// <summary>
        /// Initializes a new instance of the <see cref="ScheduledProbe"/> class.
        /// </summary>
    ...
    /// <summary>
    /// An observer that records all values, errors, and completion counts.
    /// </summary>
    /// <typeparam name="T">The type of the observed values.</typeparam>
    private sealed class RecordingObserver<T> : IObserver<T>
    {
        ...
        /// <summary>
        /// Gets the number of completion callbacks observed.
        /// </summary>
        public int Completed { get; private set; }

        /// <summary>
        /// Records a completion callback.
        /// </summary>
        public void OnCompleted() => Completed++;

        /// <summary>
        /// Records an error callback.
        /// </summary>
        /// <param name="error">The error to record.</param>
        public void OnError(Exception error) => Errors.Add(error);

        /// <summary>
        /// Records a value callback.
        /// </summary>
