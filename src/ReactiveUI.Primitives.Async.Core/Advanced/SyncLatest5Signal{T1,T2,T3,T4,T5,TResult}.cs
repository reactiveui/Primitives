...
public sealed class SyncLatest5Signal<T1, T2, T3, T4, T5, TResult> : IObservableAsync<TResult>
{
    ...
    public SyncLatest5Signal(SyncLatest5State<T1, T2, T3, T4, T5> sources, Func<T1, T2, T3, T4, T5, TResult> selector)
    {
        Sources = sources;
        Selector = selector;
    }
    ...
    private Func<T1, T2, T3, T4, T5, TResult> Selector { get; }
    ...
    {
        SyncLatest5Coordinator<T1, T2, T3, T4, T5, TResult> subscription = new(observer, Sources, Selector);
