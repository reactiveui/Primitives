...
public sealed class SyncLatest4Signal<T1, T2, T3, T4, TResult> : IObservableAsync<TResult>
{
    ...
    public SyncLatest4Signal(SyncLatest4State<T1, T2, T3, T4> sources, Func<T1, T2, T3, T4, TResult> selector)
    {
        Sources = sources;
        Selector = selector;
    }
    ...
    {
        SyncLatest4Coordinator<T1, T2, T3, T4, TResult> subscription = new(observer, Sources, Selector);
