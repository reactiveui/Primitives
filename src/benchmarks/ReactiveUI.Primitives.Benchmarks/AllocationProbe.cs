...
internal static class AllocationProbe
{
    ...
    private static void ProbeStatefulOperators(IntSignalWitness observer)
    {
        ...
        Row(
            "UniqueBy",
            () => Signal.Sequence(0, Count).UniqueBy(static x => x / KeyDivisor).Subscribe(observer).Dispose());
        ...
        Row(
            "TakeWhile",
            () => Signal.Sequence(0, Count).TakeWhile(static x => x < TakeWhileBound).Subscribe(observer).Dispose());
        Row(
            "SkipWhile",
            () => Signal.Sequence(0, Count).SkipWhile(static x => x < Window).Subscribe(observer).Dispose());
    }
    ...
    private static void ProbeProjection(IntSignalWitness observer)
    {
        ...
        Row(
            "Map+Keep",
            () => Signal.Sequence(0, Count).Map(static x => x + 1).Keep(static x => (x & 1) == 0).Subscribe(observer)
                .Dispose());
        Row(
            "Zip",
            () => Signal.Pair(Signal.Sequence(0, Count), Signal.Sequence(0, Count), static (l, r) => l + r)
                .Subscribe(observer).Dispose());
        Row(
            "WithLatest (Latch)",
            () => Signal.Sequence(1, Count).Latch(Signal.Sequence(RightStart, Count), static (l, r) => l + r)
                .Subscribe(observer).Dispose());
        Row(
            "CombineLatest (SyncLatest)",
            () => Signal.Sequence(0, Count).SyncLatest(Signal.Sequence(RightStart, Count), static (l, r) => l + r)
                .Subscribe(observer).Dispose());
        Row(
            "ForkJoin",
            () => Signal.Sequence(0, Count).ForkJoin(Signal.Sequence(RightStart, Count), static (l, r) => l + r)
                .Subscribe(observer).Dispose());
        Row(
            "FlatMap",
            () => Signal.Sequence(1, FlatMapSourceCount)
                .FlatMap(static x => Signal.Sequence(x * FlatMapMultiplier, InnerCount)).Subscribe(observer).Dispose());
    }
        ...
        CountingSignalWitness<int[]> arrayObserver)
    {
        ...
        Row(
            "Unspark (Spark->Unspark)",
            () => Signal.Sequence(0, Count).Spark().Unspark().Subscribe(observer).Dispose());
        Row(
            "TimeInterval",
            () => Signal.Sequence(0, Count).TimeInterval(Sequencer.Immediate).Subscribe(intervalObserver).Dispose());
        Row(
            "SubscribeOn (Immediate)",
            () => Signal.Sequence(0, Count).SubscribeOn(Sequencer.Immediate).Subscribe(observer).Dispose());
        ...
        IObservable<int> failSource)
    {
        ...
        Row(
            "Recover",
            () => failSource.Recover<int, Exception>(static _ => Signal.Sequence(0, Count)).Subscribe(observer)
                .Dispose());
        Row("Resume", () => failSource.Resume(Signal.Sequence(0, Count)).Subscribe(observer).Dispose());
        Row(
            "MapWith",
            () => Signal.Sequence(0, Count).MapWith(MapWithFactor, static (f, x) => x * f).Subscribe(observer)
                .Dispose());
        Row(
            "KeepWith",
            () => Signal.Sequence(0, Count).KeepWith(Window, static (t, x) => x > t).Subscribe(observer).Dispose());
        Row(
            "TapWith",
            () => Signal.Sequence(0, Count).TapWith(0, static (s, x) => _ = x + s).Subscribe(observer).Dispose());
        Row(
            "KeepNotNull",
            () => Signal.Sequence(0, Count).Map(static x => (string?)x.ToString()).KeepNotNull()
                .Subscribe(stringObserver).Dispose());
        Row(
            "KeepType",
            () => Signal.Sequence(0, Count).Map(static _ => (object?)ProbeShared).KeepType<string>()
                .Subscribe(stringObserver).Dispose());
        Row(
            "CastTo",
            () => Signal.Sequence(0, Count).Map(static _ => (object?)ProbeShared).CastTo<string>()
                .Subscribe(stringObserver).Dispose());
        Row(
            "Iterate",
            () => Signal.Iterate(0, static s => s < Count, static s => s + 1, static s => s).Subscribe(observer)
                .Dispose());
    ...
    private static void Row(string label, Action op)
    {
        ...
        Console.WriteLine($"  {label, NameColumnWidth} {perOp, ValueColumnWidth} B");
    }
}
