...
public sealed class LeasedOperationBatchTests
{
    ...
    {
        var constructor = typeof(LeasedOperationBatch).GetConstructors().Single();
        var exception = await Assert.That(() => constructor.Invoke([Guid.NewGuid(), DateTimeOffset.UnixEpoch, null])).ThrowsExactly<TargetInvocationException>();
