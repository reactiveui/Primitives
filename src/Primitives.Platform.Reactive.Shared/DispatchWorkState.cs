...
/// <summary>The run and cancel state of one scheduled work item, embedded as a mutable field by the item that owns its scheduling resources.</summary>
/// <typeparam name="TState">The scheduled state type.</typeparam>
/// <remarks>Keep the field non-readonly and call it in place, since a copy is a separate item.</remarks>
[System.Diagnostics.DebuggerDisplay("DispatchWorkState: IsDisposed = {IsDisposed}")]
public record struct DispatchWorkState<TState>
{
    ...
    /// <summary>Initializes a new instance of the <see cref="DispatchWorkState{TState}"/> struct.</summary>
    ...
    public DispatchWorkState(IScheduler scheduler, TState state, Func<IScheduler, TState, IDisposable> action)
    {
    ...
    public void ReleaseCanceledResult()
    {
        if (!IsDisposed)
        {
            return;
    ...
    public bool TryClaimDispose() => Interlocked.Exchange(ref _isDisposed, 1) == 0;

    /// <summary>Disposes whatever the started action returned, so a cancel arriving after the start tears it down.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void ReleaseStartedWork() => Interlocked.Exchange(ref _disposable, Disposable.Empty)?.Dispose();
}
