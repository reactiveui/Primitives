...
public partial class CombineLatestOperatorTests
{
    ...
    public async Task WhenCombineLatest2Sources_Source1Errors_ThenFailurePropagates()
    {
        ...
        await using var sub = await s1.Values.CombineLatest(s2.Values, static (a, b) => a + b)
            .SubscribeAsync(
                static (_, _) => default,
                null,
                r =>
    ...
    public async Task WhenCombineLatest2Sources_DisposedDuringEmission_ThenNoError()
    {
        ...
        List<int> results = [];
        var sub = await s1.Values.CombineLatest(s2.Values, static (a, b) => a + b)
            .SubscribeAsync(
                (x, _) =>
                {
                    results.Add(x);
    ...
    public async Task WhenCombineLatest3Sources_DisposedDuringEmission_ThenNoError()
    {
        ...
        List<int> results = [];
        var sub = await s1.Values.CombineLatest(s2.Values, s3.Values, static (a, b, c) => a + b + c)
            .SubscribeAsync(
                (x, _) =>
                {
                    results.Add(x);
