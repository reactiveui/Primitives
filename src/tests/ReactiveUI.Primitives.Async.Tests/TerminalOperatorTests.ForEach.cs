...
public partial class TerminalOperatorTests
{
    ...
    [Test]
    public async Task WhenForEachAsyncWithNullSyncAction_ThenThrowsArgumentNullException() => await Assert.That(async () => await SignalAsync.Return(1).ForEachAsync((Action<int>)null!)).ThrowsExactly<ArgumentNullException>();

    /// <summary>Tests ForEachAsync with null async action throws ArgumentNullException.</summary>
    /// <returns>A task representing the asynchronous operation.</returns>
    [Test]
    public async Task WhenForEachAsyncWithNullAsyncAction_ThenThrowsArgumentNullException() => await Assert.That(async () => await SignalAsync.Return(1).ForEachAsync(null!)).ThrowsExactly<ArgumentNullException>();
    ...
    public async Task WhenForEachAsyncSourceFails_ThenThrows()
    {
        ...
        await Assert.That(async () => await SignalAsync.Throw<int>(error).ForEachAsync((_, _) => default)).ThrowsExactly<InvalidOperationException>();
    }
    ...
    public async Task WhenToAsyncEnumerableSourceFails_ThenThrows()
    {
        ...
        {
            await foreach (var item in SignalAsync.Throw<int>(error).ToAsyncEnumerable(() => Channel.CreateUnbounded<int>()))
            {
                _ = item;
            }
        }).ThrowsExactly<InvalidOperationException>();
    ...
    [Test]
    public void WhenWrapWithNullObserver_ThenThrowsArgumentNullException() => Assert.Throws<ArgumentNullException>(() => SignalAsyncExtensions.Wrap<int>(null!));

    /// <summary>Verifies the async-callback <c>ForEachAsync</c> overload throws when the callback delegate is null.</summary>
    /// <returns>A <see cref = "Task"/> representing the asynchronous test operation.</returns>
    [Test]
    public async Task WhenForEachAsyncCallbackNull_ThenThrowsArgumentNullException() => await Assert.That(async () => await SignalAsync.Return(1).ForEachAsync(null!, CancellationToken.None)).ThrowsExactly<ArgumentNullException>();
}
