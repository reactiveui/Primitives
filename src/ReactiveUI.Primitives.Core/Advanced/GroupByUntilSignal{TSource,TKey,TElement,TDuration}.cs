...
/// <summary>Cold signal that splits a source into keyed groups that each end when a duration signal fires.</summary>
...
[System.Diagnostics.DebuggerDisplay("GroupByUntilSignal: Source = {_source}")]
public sealed class GroupByUntilSignal<TSource, TKey, TElement, TDuration> : IObservable<GroupedSignal<TKey, TElement>>
    where TKey : notnull
{
    ...
    /// <summary>Initializes a new instance of the <see cref="GroupByUntilSignal{TSource, TKey, TElement, TDuration}"/> class.</summary>
    ...
    public IDisposable Subscribe(IObserver<GroupedSignal<TKey, TElement>> observer)
    {
        ...
        GroupByUntilWitness<TSource, TKey, TElement, TDuration> sink =
            new(observer, _keySelector, _elementSelector, _durationSelector, _comparer, _capacity);
