...
public static partial class SignalAsyncExtensions
{
    ...
    internal class MergeCoordinator<T> : IAsyncDisposable
    {
        ...
        internal class MergeBranchWitness(MergeCoordinator<T> parent) : WitnessAsync<T>
        {
    ...
    internal sealed class MergeEnumerableSignal<T>(IEnumerable<IObservableAsync<T>> sources) : SignalAsync<T>
    {
        ...
        internal sealed class MergeSequenceCoordinator : IAsyncDisposable
        {
            ...
            internal sealed class MergeBranchWitness(MergeSequenceCoordinator parent) : WitnessAsync<T>
            {
                /// <inheritdoc/>
                protected override ValueTask OnNextAsyncCore(T value, CancellationToken cancellationToken)
                    => parent.RelayNextAsync(value, cancellationToken);
