...
{
    /// <summary>Verifies that CombineLatest12 disposes on subscription failure (catch block).</summary>
    ...
    public async Task WhenCombineLatest12SubscriptionThrows_ThenDisposesAndRethrows()
    {
        ...
        await Assert.That(async () => await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, s9.Values, s10.Values, s11.Values, throwingSrc, (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9 + v10 + v11 + v12).SubscribeAsync((_, _) => default, null)).ThrowsExactly<InvalidOperationException>();
    }

    /// <summary>Verifies that CombineLatest12 OnNextCombined guard returns when disposed.</summary>
    /// <returns>A <see cref = "Task"/> representing the asynchronous test operation.</returns>
    [Test]
    [SuppressMessage("Major Code Smell", "S107:Methods should not have too many parameters", Justification = "Test Reasons")]
    [SuppressMessage("Major Code Smell", "S138:Methods should not have too many lines", Justification = "Smoke test inherently lists N Signals + per-source calls; splitting would obscure the under-test sequence.")]
    public async Task WhenCombineLatest12DisposedBeforeCombine_ThenOnNextCombinedIsGuarded()
    {
        ...
        var sub = await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, s9.Values, s10.Values, s11.Values, s12.Values, (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9 + v10 + v11 + v12).SubscribeAsync(
            (x, _) =>
        {
            results.Add(x);
            return default;
    ...
    /// <summary>Verifies that CombineLatest12 forwards a source error to the downstream observer.</summary>
    /// <returns>A <see cref = "Task"/> representing the asynchronous test operation.</returns>
    [Test]
    [SuppressMessage("Major Code Smell", "S107:Methods should not have too many parameters", Justification = "Test Reasons")]
    [SuppressMessage("Major Code Smell", "S138:Methods should not have too many lines", Justification = "Smoke test inherently lists N Signals + per-source calls; splitting would obscure the under-test sequence.")]
    public async Task WhenCombineLatest12OneSourceErrors_ThenCombinedErrorForwarded()
    {
        ...
        await using var sub = await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, s9.Values, s10.Values, s11.Values, s12.Values, (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9 + v10 + v11 + v12).SubscribeAsync((_, _) => default, (ex, _) =>
        {
            receivedError = ex;
    ...
    /// <summary>Verifies that CombineLatest12 produces the selector's result once every source has emitted at least once.</summary>
    /// <returns>A <see cref = "Task"/> representing the asynchronous test operation.</returns>
    [Test]
    [SuppressMessage("Major Code Smell", "S107:Methods should not have too many parameters", Justification = "Test Reasons")]
    [SuppressMessage("Major Code Smell", "S138:Methods should not have too many lines", Justification = "Smoke test inherently lists N Signals + per-source calls; splitting would obscure the under-test sequence.")]
    public async Task WhenCombineLatest12AllSourcesEmit_ThenSelectorResultEmitted()
    {
        ...
        await using var sub = await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, s9.Values, s10.Values, s11.Values, s12.Values, (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9 + v10 + v11 + v12).SubscribeAsync(
            (x, _) =>
        {
            results.Add(x);
        ...
        await Assert.That(results[0]).IsEqualTo(1 + PlaceValue1 + PlaceValue2 + PlaceValue3 + PlaceValue4 + PlaceValue5 + PlaceValue6 + PlaceValue7 + PlaceValue8 + PlaceValue9 + PlaceValue10 + PlaceValue11);
    ...
    /// <summary>Verifies that CombineLatest12 completes once every source has completed.</summary>
    /// <returns>A <see cref = "Task"/> representing the asynchronous test operation.</returns>
    [Test]
    [SuppressMessage("Major Code Smell", "S107:Methods should not have too many parameters", Justification = "Test Reasons")]
    [SuppressMessage("Major Code Smell", "S138:Methods should not have too many lines", Justification = "Smoke test inherently lists N Signals + per-source calls; splitting would obscure the under-test sequence.")]
    public async Task WhenCombineLatest12AllSourcesComplete_ThenCombinedCompletes()
    {
        ...
        await using var sub = await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, s9.Values, s10.Values, s11.Values, s12.Values, (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9 + v10 + v11 + v12).SubscribeAsync((_, _) => default, null, r =>
        {
