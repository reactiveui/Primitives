...
public sealed class SyncLatest3Signal<T1, T2, T3, TResult> : IObservableAsync<TResult>
{
    ...
    public SyncLatest3Signal(SyncLatest3State<T1, T2, T3> sources, Func<T1, T2, T3, TResult> selector)
    {
        Sources = sources;
        Selector = selector;
    }
    ...
    {
        SyncLatest3Coordinator<T1, T2, T3, TResult> subscription = new(observer, Sources, Selector);
