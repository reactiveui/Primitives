    ...
    where TAbsolute : IComparable<TAbsolute>
{
    ...
    internal readonly long Timestamp => Now.UtcTicks;
    ...
    /// <param name="time">Relative time to advance the scheduler's clock by.</param>
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="time"/> is negative.</exception>
    internal void Sleep(TRelative time)
    {
        var dt = _add(_clock, time);
    ...
    internal readonly IDisposable Schedule<TState>(
        ISequencer owner,
        TState state,
    ...
    internal readonly void Schedule(ISequencer owner, IWorkItem item)
    {
    ...
    /// <exception cref="ArgumentNullException"><paramref name="item"/> is <see langword="null"/>.</exception>
    internal readonly void Schedule(ISequencer owner, IWorkItem item, long dueTimestamp)
    {
        ArgumentExceptionHelper.ThrowIfNull(item);
    ...
    /// <returns>The disposable object used to cancel the scheduled action (best effort).</returns>
    /// <exception cref="ArgumentNullException"><paramref name="action"/> is <c>null</c>.</exception>
    internal readonly IDisposable ScheduleAbsolute<TState>(
        ISequencer owner,
        TState state,
        TAbsolute dueTime,
        Func<ISequencer, TState, IDisposable> action)
    ...
    /// <returns>The disposable object used to cancel the scheduled action (best effort).</returns>
    /// <exception cref="ArgumentNullException"><paramref name="action"/> is <c>null</c>.</exception>
    internal readonly IDisposable ScheduleRelative<TState>(
        ISequencer owner,
        TState state,
        TRelative dueTime,
        Func<ISequencer, TState, IDisposable> action)
