...
/// <summary>
/// Provides extension methods for working with signals in a reactive programming context.
/// </summary>
...
/// observers.</remarks>
public static class SignalMixins
{
    /// <summary>
    /// Creates an asynchronous observer wrapper for the specified signal.
    /// </summary>
    /// <typeparam name="T">The type of the elements processed by the signal and observer.</typeparam>
    /// <param name="signal">The signal to wrap as an asynchronous observer. Cannot be null.</param>
    ...
    public static IObserverAsync<T> AsObserverAsync<T>(this ISignalAsync<T> signal)
    {
        ArgumentExceptionHelper.ThrowIfNull(signal);

        return new SignalAsyncObserver<T>(signal);
    }
    ...
    /// <param name="this">The source signal whose values are to be mapped.</param>
    ...
    public static ISignalAsync<T> MapValues<T>(
        this ISignalAsync<T> @this,
    ...
    /// <summary>
    /// A signal that applies a transformation to the observable values of the source signal.
    /// </summary>
    ...
    /// <summary>
    /// An asynchronous observer that forwards all notifications to the wrapped signal.
    /// </summary>
    ...
    internal sealed class SignalAsyncObserver<T>(ISignalAsync<T> signal) : ObserverAsync<T>
    {
        ...
        /// <summary>
        /// Forwards the error to the wrapped signal, replacing the cancellation token with
