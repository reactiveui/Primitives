...
public class FirstMatchFromCandidatesAsyncTests
{
    ...
    {
        Subject<string> s1 = new();
        Subject<string> s2 = new();
        Subject<string>[] subjects = [s1, s2];
        List<string> results = [];
        var completed = false;
        int[] keys = [0, 1];
        using var sub = keys.FirstMatchFromCandidates(i => subjects[i], static raw => raw, static _ => false, Fallback)
    ...
    public async Task WhenAsyncProjectionErrors_ThenSkipsAndContinues()
    {
        ...
        int[] keys = [0, 1];
        using var sub = keys
            .FirstMatchFromCandidates(i => subjects[i], static raw => raw, static raw => raw == MatchRaw, Fallback)
    ...
    public async Task WhenAsyncSinkDisposedBeforeEmission_ThenNoDownstreamNotifications()
    {
        ...
        var completed = false;
        string[] keys = ["k"];
        var sub = keys.FirstMatchFromCandidates(_ => s, static raw => raw, static _ => true, Fallback)
            .Subscribe(results.Add, () => completed = true);
        sub.Dispose();
        s.OnNext("ignored");
        s.OnCompleted();
    ...
    {
        using Subject<string> asyncFirst = new();
        List<string> results = [];
        string[] keys = ["async-open", "throw-key", "match"];
        using var sub = keys.FirstMatchFromCandidates(
            key => key switch
            {
