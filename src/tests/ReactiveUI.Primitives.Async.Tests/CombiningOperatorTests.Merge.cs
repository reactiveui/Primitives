...
public partial class CombiningOperatorTests
{
    ...
    public async Task WhenMergeWithMaxConcurrency_ThenRespectsLimit()
    {
        ...
        var source = SignalAsync.Range(1, 5).Select(i =>
            SignalAsync.CreateAsBackgroundJob<int>(
                async (obs, ct) =>
                {
                    lock (_gate)
                    {
    ...
    public async Task WhenMergeEnumerableInnerCompletesAsynchronously_ThenAwaitsSubscriptionFinished()
    {
        ...
        Result? completionResult = null;
        await using var sub = await new[] { innerSignal.Values }.Merge()
            .SubscribeAsync(
                (_, _) => default,
                null,
                result =>
    ...
    /// <summary>Tests that MergeEnumerableSignal SubscribeAsyncCore catch block disposes and rethrows.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous test operation.</returns>
    [Test]
    ...
    {
        var source = SignalAsync.Range(1, 4).Select(i =>
            SignalAsync.CreateAsBackgroundJob<int>(
                async (obs, ct) =>
                {
                    await obs.OnNextAsync(i, ct);
    ...
    /// <summary>Verifies the <see cref="SignalAsyncExtensions.MergeCoordinator{T}.RelayNextIfActiveAsync"/>
    ...
    /// <summary>Verifies the <see cref="SignalAsyncExtensions.MergeCoordinator{T}.RelayErrorIfActiveAsync"/> inside-gate after-dispose guard.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous test operation.</returns>
    [Test]
