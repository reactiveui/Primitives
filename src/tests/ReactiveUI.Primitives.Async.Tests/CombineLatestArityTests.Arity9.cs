...
public partial class CombineLatestArityTests
{
    ...
    public async Task WhenCombineLatest9SubscriptionThrows_ThenDisposesAndRethrows()
    {
        ...
        await Assert.That(async () => await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, throwingSrc, (v1, v2, v3, v4, v5, v6, v7, v8, v9) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9).SubscribeAsync((_, _) => default, null)).ThrowsExactly<InvalidOperationException>();
    }
    ...
    public async Task WhenCombineLatest9DisposedBeforeCombine_ThenOnNextCombinedIsGuarded()
    {
        ...
        var sub = await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, s9.Values, (v1, v2, v3, v4, v5, v6, v7, v8, v9) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9).SubscribeAsync(
            (x, _) =>
        {
            results.Add(x);
            return default;
    ...
    public async Task WhenCombineLatest9OneSourceErrors_ThenCombinedErrorForwarded()
    {
        ...
        await using var sub = await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, s9.Values, (v1, v2, v3, v4, v5, v6, v7, v8, v9) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9).SubscribeAsync((_, _) => default, (ex, _) =>
        {
            receivedError = ex;
    ...
    public async Task WhenCombineLatest9AllSourcesEmit_ThenSelectorResultEmitted()
    {
        ...
        await using var sub = await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, s9.Values, (v1, v2, v3, v4, v5, v6, v7, v8, v9) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9).SubscribeAsync(
            (x, _) =>
        {
            results.Add(x);
        ...
        await Assert.That(results[0]).IsEqualTo(1 + PlaceValue1 + PlaceValue2 + PlaceValue3 + PlaceValue4 + PlaceValue5 + PlaceValue6 + PlaceValue7 + PlaceValue8);
    ...
    public async Task WhenCombineLatest9AllSourcesComplete_ThenCombinedCompletes()
    {
        ...
        await using var sub = await s1.Values.CombineLatest(s2.Values, s3.Values, s4.Values, s5.Values, s6.Values, s7.Values, s8.Values, s9.Values, (v1, v2, v3, v4, v5, v6, v7, v8, v9) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9).SubscribeAsync((_, _) => default, null, r =>
        {
