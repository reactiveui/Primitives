...
public sealed class SyncLatest2Signal<T1, T2, TResult> : IObservableAsync<TResult>
{
    ...
    public SyncLatest2Signal(SyncLatest2State<T1, T2> sources, Func<T1, T2, TResult> selector)
    {
        Sources = sources;
        Selector = selector;
    }
    ...
    {
        SyncLatest2Coordinator<T1, T2, TResult> subscription = new(observer, Sources, Selector);
